...
#define EEPROM_SIZE 2              // Dimensione della EEPROM da utilizzare (2 byte)
#define EEPROM_ANIMATION_ADDR 0    // Indirizzo dove salvare l'animazione
#define EEPROM_BRIGHTNESS_ADDR 1   // Indirizzo dove salvare la luminosità
...
// ============================================================================
// FUNZIONI PER LA GESTIONE DELLA MEMORIA EEPROM
// ============================================================================

// Salva l'animazione corrente nella EEPROM
void saveAnimation() {
  EEPROM.write(EEPROM_ANIMATION_ADDR, currentAnimation);
  EEPROM.commit();  // Necessario per ESP32 per rendere permanente la scrittura
  Serial.print(">>> Animazione salvata in EEPROM: ");
  Serial.println(currentAnimation);
}

// Carica l'ultima animazione salvata dalla EEPROM
int loadAnimation() {
  int savedAnimation = EEPROM.read(EEPROM_ANIMATION_ADDR);
  
  // Verifica che il valore letto sia valido
  // Se non è valido (primo avvio o EEPROM corrotta), ritorna 0 (bianco fisso)
  if (savedAnimation >= 0 && savedAnimation < NUM_ANIMATIONS) {
    Serial.print(">>> Animazione caricata da EEPROM: ");
    Serial.println(savedAnimation);
    return savedAnimation;
  } else {
    Serial.println(">>> EEPROM non inizializzata, uso animazione predefinita (0)");
    return 0;
  }
}

// Salva la luminosità corrente nella EEPROM
void saveBrightness() {
  EEPROM.write(EEPROM_BRIGHTNESS_ADDR, brightness);
  EEPROM.commit();  // Necessario per ESP32 per rendere permanente la scrittura
  Serial.print(">>> Luminosità salvata in EEPROM: ");
...
// Carica l'ultima luminosità salvata dalla EEPROM
int loadBrightness() {
  int savedBrightness = EEPROM.read(EEPROM_BRIGHTNESS_ADDR);
  
  // Verifica che il valore letto sia valido (tra 10 e 255)
  // Se non è valido (primo avvio o EEPROM corrotta), ritorna 255 (massima luminosità)
  if (savedBrightness >= 10 && savedBrightness <= 255) {
    Serial.print(">>> Luminosità caricata da EEPROM: ");
    ...
    Serial.println(">>> Luminosità EEPROM non valida, uso valore predefinito (255)");
    return 255;
  }
}
...
void setup() {
  ...
  pixels.setBrightness(brightness);  // Imposta la luminosità caricata dalla EEPROM
...
void handleTB1() {
  ...
  if (TB1_pressed && !TB1_aux) {
    ...
    if (lampOn) {
      ...
      // L'animazione è già impostata a quella salvata in EEPROM
      // (caricata durante il setup)
      
      // La luminosità è già impostata a quella salvata in EEPROM
      // (caricata durante il setup)
...
void handleTB2() {
  ...
  if (!TB2_pressed && TB2_aux) {
    ...
    if (pressDuration < LONG_PRESS_TIME && lampOn) {
      ...
      // Salva la nuova animazione nella EEPROM
      saveAnimation();
